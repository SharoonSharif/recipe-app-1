// src/routes/app/index.tsx - Fixed with comprehensive error handling
...
function AppHome() {
  ...
  const handleRetry = useCallback(() => {
    setError(null)
    // Force refresh of queries by changing the component state
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4">
          ...
          <div className="text-sm text-muted-foreground">
            {isSessionLoading ? 'Checking authentication...' : 'Loading user data...'}
          </div>
        </div>
      </div>
    )
        ...
        <Card className="max-w-md mx-auto">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              Please log in to access your recipes.
            </p>
  ...
  const handleDeleteRecipe = async (id: Id<'recipes'>) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
      setError(`Failed to delete recipe: ${errorMessage}`)
    }
  }
  ...
  const handleCreateShoppingList = async (name: string, selectedRecipeIds: Id<'recipes'>[]) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
      setError(`Failed to create shopping list: ${errorMessage}`)
    }
  }
  ...
  const handleDeleteShoppingList = async (id: Id<'shoppingLists'>) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
      setError(`Failed to delete shopping list: ${errorMessage}`)
    }
  }
